...
研究报告、分析记录、数据汇总与派生数据集作为中间结果直接公开，不必等它成熟为判断：

- 可公开：作者撰写的报告与宽表、审查后的汇总统计、可复算的派生数据集。发布时标注快照日期、覆盖范围与已知缺口，并链接它支撑的知识单元或主题。
- 不可公开：第三方原文、含个人信息的原始爬取数据、未完成权限审查的导出。它们只进本地隔离区；公开层发布其汇总与口径（见“数据发布”）。
- 落点：独立数据集放 `knowledge/data/`（带日期快照）；报告与宽表作为文章附件（`assets/downloads/`）或主题页链接；每次发布后在 `knowledge/changes/` 留一条记录。
- 中间结果会被替代而不是悄悄修改：新版本用新日期文件，旧版本保留，正文里指明被哪个版本接替。
