...
RAG 仍然可以负责从知识库、代码库或 Memory 中生成候选。变化在它之后：检索结果还要和任务状态、固定指令、工具、Skill 与权限一起经过选择。
...
RAG 会成为这套系统中的一个候选生成器，而不是被淘汰的旧方案。
...
同一个模型，拿到不同工具，会形成不同计划；带着不同 Memory，会延续不同历史；受到不同权限约束，会停在不同边界；每一步看到不同的 Context，最后就会表现得像不同的 Agent。

所以 RAG 之后的问题，不是还能往窗口里塞多少东西。
