...
模型跑分、机器人动作和硬件参数依然重要，但客户已经开始追问更多：长期运行的表现如何，失败时谁来接管，一次成功任务究竟要花多少钱？行业还没有进入规模化收获期，却开始认真为“把事做成”买单。
...
[晚点聊 LateTalk 的 2026 年 Q2 季报](https://www.bilibili.com/video/BV1Mhgz6QEHA/)解释了 Coding 的另一层意义：它既是眼下最明确的收入入口，也可能成为下一代 AI 的研发基础设施。
...
WAIC 上，PPIO 披露 Agent 沙箱上线一年后业务规模增长 123 倍。这个数字来自公司口径，仍需要客户侧验证；[访谈中提出的评估方式](https://mp.weixin.qq.com/s/gH7pvWzd4mlYWWEKJSS2hQ)更完整：不要只看一次模型调用有多便宜，要把上下文、工具调用、失败重试和人工检查全部算进一次成功任务。
...
[阶跃在 WAIC 展示的 Agentic OS](https://mp.weixin.qq.com/s/2HERNh-u9A8NxWk0UynJJw)采用了类似思路。它还没有证明大规模量产和长期留存，但已经提出跨 App 操作之外的信任问题：操作能力需要建立在用户授权之上。
...
领先模型公司确实拥有一套复利结构：更强的模型带来更多用户，更多使用产生更多失败样本和偏好数据，收入支持算力和人才，研发工具又帮助开发下一代模型。Coding 在这套结构里很特殊，因为它既产生收入，也提高研发效率。

但过去三年的模型竞争一直在交替领先。人才会流动，开源模型在用成本和可控性进入企业，掌握专用数据、领域评测和客户流程的垂直公司仍有机会在局部建立更强的系统，而企业销售、权限治理、硬件量产和售后服务需要另一套长期积累。因此，我对“强者愈强”的判断是：收入、分发和研发效率会先集中，技术领先未必永久集中；只有当某家公司率先实现可迁移、可持续的 RSI，现有平衡才可能被真正打破。
...
WAIC 2026 之后，Agent、机器人、AI 硬件和算力都开始接受更严格的交付检验。能够留下来的公司会持续记录失败，用这些数据让交付更稳定、也更便宜。哪些系统能长期、稳定地把事情做好，比 AI 会什么更值得跟踪。
