---
layout: post
title: "竞品分析为什么不该从参数表开始：软件与硬件是两套比较逻辑"
subtitle: "参数表只能比较已经被定义的东西。真正的竞品分析，要先解释用户在什么情境下，从什么方案切换到什么方案。"
...
参数表最大的问题不是参数，而是它把一个选择问题过早地变成了产品问题。
...
因此，软件竞品分析的基本单位不该是“功能”，而应该是**一条端到端工作流**。
...
软件竞争的关键不是“今天谁多一个功能”，而是谁能更低成本地进入真实工作流，承接用户已有状态，并在持续变化中保持可靠。
...
产品在哪里试用和购买，多久到货，故障后是换新、寄修还是上门，备件能供应多久？FTC 对消费品保修的说明把覆盖期限、维修范围、处理流程和运输成本都列为购买时应检查的内容。售后不是成交后的附属环节，而是硬件价值的一部分。

### 6. 全生命周期成本

购买价格只是起点，还要计算配件、耗材、订阅、维修、闲置、残值和替换周期。对于企业硬件，还可能包括部署、培训、场地、安全和停机成本。

硬件竞争不是把最好看的样机做出来，而是把足够一致、可靠、可维护的系统交付给大量用户，并对它的整个生命周期负责。
