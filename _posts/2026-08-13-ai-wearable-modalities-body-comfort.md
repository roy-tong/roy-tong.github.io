...
subtitle: "真正需要比较的，是输入与输出模态、控制方式、身体位置和长期佩戴成本组成的整套系统。"
...
谈到 AI Wearable，行业最容易使用的分类是眼镜、耳机、戒指、手表和挂坠。
...
因此，AI Wearable 的竞争不应被简化为“眼镜对耳机”。更有解释力的分析单位是四层组合：
...
多模态不是“产品同时拥有摄像头、麦克风和大模型”。

输入模态决定系统能理解什么，输出模态决定系统能怎样回应。两者必须与具体任务一起比较。

W3C 很早就把语音、触摸、键盘和手写等视为不同输入，把显示、语音、音频和触觉视为不同输出。进入 AI Wearable 时代，新的地方不是这些模态突然出现，而是模型能够在它们之间理解语义、保持上下文并生成回应。
...
这不是把所有入口堆在一起，而是根据动作的频率、风险、隐私和认知负担分工。
...
> AI Wearable 可能不是一件独立设备，而是一套分布在眼睛、耳朵、手指、手腕和手机之间的个人交互系统。
...
“上下文密度”不是传感器数量，而是某个位置能否持续接近用户正在看、听、说、做和决定的事情。

五官附近尤其重要，因为它同时接近现实输入和人的主要感知通道。眼镜获得与视线接近的第一视角，耳机靠近听觉并能私密输出，嘴部附近最接近语言意图。脖子和胸前则提供另一种可能：把部分重量、摄像头、扬声器、电池或计算从面部移开，换取更大的承载空间。

但身体位置不是一块被抢到就永久拥有的“地盘”。它是一份持续许可。设备只有在用户愿意戴、允许感知、并能在真实场景使用时，才真正拥有上下文。

这也意味着，最强的竞争对手经常仍是手机。手机虽然启动摩擦更大，却拥有成熟显示、输入、算力、连接和应用生态。Wearable 必须证明，它在某些高频或关键时刻减少的摩擦，足以覆盖新增设备带来的佩戴、充电和管理成本。

## 四、舒适性不是工业设计指标，而是产品能力
...
因此，舒适性不是产品完成后的优化项，而是决定传感器、算力、结构、交互和商业模式的一级约束。
...
这四种路线并不互斥，也不是固定终局。它们只是帮助产品团队看清：增加一项模态或一个控制器，不是增加一个功能，而是改变整套系统的收益和成本。

一个产品也不需要覆盖所有路线。真正有用的定位应说明：服务谁、在什么情境、完成什么任务、为什么这组模态和身体位置比手机或现有设备更好，以及不适合谁。
...
AI Wearable 的验证，不是问用户喜不喜欢一种形态，而是检验他是否愿意为一组持续可用的任务，把某套系统长期留在身体上。

## 结语：下一代入口是一套组合，不是一件首饰
...
最终胜出的不一定是传感器最多、显示最强或模型最大的设备，而是能把模态、交互、身体位置与舒适性组合成一个低摩擦闭环的产品。
